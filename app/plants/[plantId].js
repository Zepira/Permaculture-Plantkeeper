...
export default PlantDetail = () => {
  ...
  const calculateDaysUntilActionDue = (lastActionDate, frequency) => {
    if (lastActionDate) {
      const daysSinceAction = Math.round(
        Math.abs((today - lastActionDate.toDate()) / (1000 * 60 * 60 * 24)),
      );
    ...
    useCallback(() => {
      ...
      if ((userPlants, plants)) {
        ...
        if (originalUserPlant) {
          ...
          setIsUserPlant(true);
          setVariety(
            originalUserPlant?.variety ? originalUserPlant.variety : null,
          );
          const daysSincePlantedDate = Math.round(
            Math.abs(
              (today - originalUserPlant.plantedDate.toDate()) /
                (1000 * 60 * 60 * 24),
            ),
          );
          ...
          switch (originalUserPlant.growthStage) {
            case 1:
              setGrowthStageIcon("seed");
              setDaysTillNextPlantStage(plantDetail.daysToSprout);
              console.log(
                "seed sprouts in ",
                plantDetail.daysToSprout,
                "days since planted: ",
                Math.round(
                  Math.abs(
                    (today - originalUserPlant.plantedDate.toDate()) /
              ...
              break;
            case 2:
              setGrowthStageIcon("sprout");
              setDaysTillNextPlantStage(plantDetail.sproutToHarvest);
              console.log(
                "sprout ready to harvest in ",
                plantDetail.sproutToHarvest,
                "days since planted: ",
                Math.round(
                  Math.abs(
                    (today - originalUserPlant.plantedDate.toDate()) /
              ...
              break;
            case 3:
              setGrowthStageIcon("flower-tulip");
              setDaysTillNextPlantStage(0);
              break;
            default:
              setGrowthStageIcon("sprout");
              setDaysTillNextPlantStage(0);
          ...
          const daysSinceLastWatered = calculateDaysUntilActionDue(
            originalUserPlant.lastWateredDate,
            waterFrequency,
          );
          setDaysToWaterIn(daysSinceLastWatered);

          const daysSinceLastFertilised = calculateDaysUntilActionDue(
            originalUserPlant.lastFertilisedDate,
            fertilisingFrequency,
          );
          setDaysToFertiliseIn(daysSinceLastFertilised);

          const daysSinceLastWeeded = calculateDaysUntilActionDue(
            originalUserPlant.lastWeededDate,
            weedingFrequency,
          );
  ...
  const updatePlantAction = (actionType) => {
    console.log("UPDATE PLANT", actionType);
    switch (actionType) {
      case "water":
  ...
  const PlantingCalendar = ({ plantSowingDates }) => {
    const calculateGrowingCalendar = () => {
      const monthArray = [];
      const sowingDates = plantSowingDates.find(
        (x) => x.climateZone === 4,
      ).sowingDates;

      Array.from({ length: 12 }, (_, k) => {
        let isMidMonth = false;
        const isEndMonth = false;
        const isStartMonth = false;
      ...
      const updatedMonthAray = monthArray;
      ...
      <View style={{ marginTop: 10 }}>
        <View style={{ flexDirection: "row", paddingHorizontal: 7 }}>
          {Array.from({ length: 12 }, (_, k) => {
            ...
            return (
              <View
                key={k}
                style={{ flex: 1, justifyContent: "flex-end", marginBottom: 2 }}
              >
                {isCurrentMonth && (
                  <Avatar.Icon
                    icon="triangle"
        ...
        >
          {calculateGrowingCalendar().map((month) => {
            const isGrowingMonth =
              month.isStartMonth | month.isEndMonth | month.isMidMonth;

            return (
              <View
                key={month.monthKey}
                style={{
                  backgroundColor: isGrowingMonth
                    ? colours.primary
                    : colours.transparent,
                  flex: 1,
                  height: 20,
                  borderBottomLeftRadius: month.isStartMonth ? 20 : 0,
      ...
      {isLoading ? (
        ...
        <>
          {plant && (
            <>
              <ImageBackground
                source={{ uri: variety ? variety.images[0] : plant.images[0] }}
                ...
                <SafeAreaWrapper>
                  <TopActionButtonContainer>
                    ...
                    {isUserPlant && (
                      <TopActionButton
                        onPressAction={() =>
                          router.replace("/plants/edit/" + userPlant.id)
                        }
                        icon="cog"
                      />
                    )}
                    ...
                    <>
                            ...
                            >
                              ...
                              >
                                <CircularProgressIndicator
                                  percentage={daysToWaterIn}
                                  max={waterFrequency}
                                />
                                <Text>
                                  {daysToWaterIn > 0
                                    ? `Water in ${daysToWaterIn} days`
                                    : `Water now`}
                                </Text>
                              </Pressable>

                              <TouchableOpacity
                                onPress={() => updatePlantAction("fertilise")}
                              >
                                <CircularProgressIndicator
                                  percentage={daysToFertiliseIn}
                                  max={fertilisingFrequency}
                                />
                                <Text>
                                  {daysToFertiliseIn > 0
                                    ? `Fertilise in ${daysToFertiliseIn} days`
                                    : `Fertilise now`}
                                </Text>
                              </TouchableOpacity>
                              <TouchableOpacity
                                onPress={() => updatePlantAction("weed")}
                              >
                                <CircularProgressIndicator
                                  percentage={daysToWeedIn}
                                  max={weedingFrequency}
                                />
                                <Text>
                                  {daysToWeedIn > 0
                                    ? `Weed in ${daysToWeedIn} days`
                                    : `Weed now`}
                                </Text>
                              </TouchableOpacity>
                            </View>
                      ...
                      {isUserPlant && (
                        ...
                        >
                          <CircularProgressIndicator
                            percentage={
                              daysSincePlanted > daysTillNextPlantStage
                                ? daysTillNextPlantStage
                                : daysSincePlanted
                            }
                            max={daysTillNextPlantStage}
                            radius={100}
                            icon={growthStageIcon}
                          />
                        </View>
                      )}
                        ...
                        <>
                          <Button
                            onPress={() =>
                              router.push({
                                pathname: "/form",
                                params: { formType: "addPlant", id: plant.id },
                              })
                            }
                          >
                            Add to Garden
